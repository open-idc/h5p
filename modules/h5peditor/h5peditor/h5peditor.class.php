  ...
  public function getLibraryData($machineName, $majorVersion, $minorVersion, $languageCode, $prefix = '') {
    ...
    if (!empty($files['scripts'])) {
      foreach ($files['scripts'] as $script) {
        $libraryData->javascript[$this->h5p->url . $this->h5p->path . $script->path . $script->version] = "\n" . file_get_contents($this->h5p->path . $script->path);
      }
    }
    ...
    if (!empty($files['styles'])) {
      foreach ($files['styles'] as $css) {
        ...
        $libraryData->css[$this->h5p->url . $this->h5p->path . $css->path . $css->version] = preg_replace_callback('/url\([\'"]?(?![a-z]+:|\/+)([^\'")]+)[\'"]?\)/i', 'H5peditor::buildCssPath', file_get_contents($this->h5p->path . $css->path));
      }
    }
